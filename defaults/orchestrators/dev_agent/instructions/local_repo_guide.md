...
- `defaults/orchestrators/dev_agent/instructions/*.md` - встроенные
  ...
  `repo_sync` и `local_repo_guide` (эта инструкция).
...
- `core/` - бизнес-логика (bootstrap, defaults, orchestrators, instructions,
  skills_library, rag, github_tools_rest, updater и т.д.); `dev_agent/` - пакет
...
- `.dev_agent/updates/` - рантайм-хранилище обновлений локальной установки
  ...
  публикуется.
- Инструкция `repo_sync`
  (`defaults/orchestrators/dev_agent/instructions/repo_sync.md`) -
  авторитетный workflow правки и публикации через манифест.
...
2. **Манифест обязателен.** Любая правка файла этой копии = bump его версии
   ...
   Системное версионирование `app_version`: крупный пакет изменений
   (значимые функции, расширение документации, архитектурные изменения)
   повышает ВТОРОЙ порядок версии (0.1.x → 0.2.x); мелкий пакет (точечные
   правки и исправления) - ТРЕТИЙ порядок (0.2.0 → 0.2.1). Источник истины -
   `core/version.py`; манифест `app_version` и строка версии в README
   обновляются вместе с ним; UI собирает версию из `core.version`
   динамически и отдельной правки не требует.

3. **Процедура публикации (детали - в инструкции `repo_sync`):**
   - загрузить инструкции `github_connector` и `repo_sync`;
   - **REST-only: никаких `git push` / `gh` / `curl`** - публикация только
     инструментами `ghr_*`; токены остаются в платформе, каждый пуш
     верифицируем через REST-инструменты;
   - **CHANGELOG.md никогда не публикуется - ПОСТОЯННОЕ исключение**: ни
     один инструмент загрузки (ghr_upload_file / ghr_update_file /
     ghr_batch_commit / ghr_batch_commit_paths / ghr_batch_upsert) не должен
     загружать CHANGELOG.md -
     ни по одному файлу, ни пакетом. При
     формировании пакета всегда отфильтровывай CHANGELOG.md из payload
   ...
   - публикация - ПРЕДПОЧТИТЕЛЬНО одним пакетным коммитом:
     `ghr_batch_commit` / `ghr_batch_commit_paths` / `ghr_batch_upsert` со
     списком {path, content},
     манифест входит в пакет ПОСЛЕДНИМ файлом; CHANGELOG.md в набор не
     включается;
   - пофайловая загрузка - только для одного-двух файлов или явной просьбы:
     `ghr_read_file` (актуальный `sha`) → `ghr_update_file` с sha (или
     `ghr_upload_file` для новых файлов), манифест - ПОСЛЕДНИМ вызовом;
   - **`file_versions.json` - ПОСЛЕДНИМ** (маркер завершённой публикации:
     пользователи сверяются с ним); при пакетной загрузке манифест входит
     в тот же пакет вместе с остальными файлами;
   - верификация: повторный `ghr_read_file(file_versions.json)` - сверка с
...
обязательно вместе с ней загрузить `repo_sync`
(`get_orchestrator_instruction('dev_agent', 'repo_sync')`).
