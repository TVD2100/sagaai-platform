...
3. **Процедура публикации (детали - в инструкции `repo_sync`):**
   ...
   - **CHANGELOG.md никогда не публикуется - ПОСТОЯННОЕ исключение**: ни
     один инструмент загрузки (github_upload_file / github_update_file /
   ...
   - изменённые файлы по одному: `github_read_file` (актуальный `sha`) →
     ...
     либо одним пакетным коммитом, если пользователь явно попросил
     «пакетом» (`ghr_batch_commit` / `ghr_batch_upsert` со списком
     {path, content}); CHANGELOG.md в набор не включается;
